...
exports.getAllUsers = async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT user_id, username, email, full_name, role, created_at FROM users where deleted_at IS NULL ORDER BY created_at DESC'
        );
...
exports.deleteUser = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'UPDATE users SET deleted_at = NOW() WHERE user_id = $1 RETURNING user_id, username, email, full_name, role',
            [id]
        );
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
